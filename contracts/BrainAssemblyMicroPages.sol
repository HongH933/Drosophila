...
contract BrainAssemblyMicroPages {
    ...
    bytes32 public constant IMPLEMENTATION_VERSION=keccak256("lif-int16-bitserial-assembly-codepages-v1");
    ...
    address public immutable builder;uint256 public immutable treeBase;Config public config;
    mapping(uint256=>SlotSpec) private specs;mapping(uint256=>bool) private configured;mapping(uint256=>bool) private assignedNeuron;
    mapping(uint256=>Holding) private holdings;mapping(uint256=>uint256) public occupiedToken;
    StaticMicroData public immutable staticData;
    mapping(uint256=>uint256) public slotEdgeBase;uint256 public configuredRecords;
    mapping(uint256=>mapping(uint256=>uint32)) private absoluteWeightSum;
    mapping(uint256=>mapping(uint256=>uint32)) private edgeOffset;
    ...
    mapping(uint256=>mapping(uint256=>bytes)) private states;
    mapping(uint256=>mapping(uint256=>uint8)) private spikes;
    mapping(uint256=>mapping(uint256=>bytes32)) private tree;
    mapping(uint256=>Work) private work;
    mapping(uint256=>WorkNode) private workTree;
    ...
    uint8 public committedBank;uint64 public assemblyVersion=1;uint64 public roundNonce;
    bool public isSealed;bool public active;bool public custodyFault;
    uint256 private entered=1;bool private awaiting;address private incomingFrom;uint256 private incomingId;
    ...
    constructor(address cpu,Config memory c,StaticMicroData.Config memory pagesConfig){
        staticData=new StaticMicroData(pagesConfig,msg.sender);
        if(c.chunks!=(pagesConfig.metadataBytes+8191)/8192)revert CONFIG();
