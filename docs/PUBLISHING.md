...
普通修改后运行 `npm run files:update`，再执行 `npm run files:verify`。生成脚本统一排除清单自身、.git、node_modules、.cache、results、release、.DS_Store及除.env.example外的.env*文件，避免自引用与私人载荷。只有脚本生成的摘要才进入清单。

`.github/workflows/reproduce.yml`准备Linux/Node22.18.0流程，安装后以Linux独立网络命名空间执行实际两步和恢复，只上传小型报告；不使用钱包/RPC秘密。配置存在不代表运行通过，本轮远程状态为`WORKFLOW_PREPARED_NOT_RUN`。用户自行审阅并push后，才能从Actions真实结果更新这项状态。

本轮修改尚未推送。公开基线以匿名HTTPS克隆，干净测试另行应用记录了SHA256的补丁；执行源码指纹排除文档与报告，并包含所有执行文件及数据锚点。文档收尾不改变核心/快照/模型/预期身份，不必重复十亿次位操作。
